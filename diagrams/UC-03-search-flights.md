...
```mermaid
flowchart TD
  A[Guest or User submits search: origin, destination, date, passengers] --> B{Cache hit for criteria?}
...
sequenceDiagram
  ...
  U->>API: GET /flights/search?origin&destination&date&passengers
...
```
function searchFlights(origin, destination, date, passengers, customerId):
    cacheKey = buildCacheKey("flight", origin, destination, date, passengers)
